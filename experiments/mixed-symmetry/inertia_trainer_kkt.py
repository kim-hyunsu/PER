...
def main(args):
    ...
    for trial in range(args.trials):
        watermark = "kkt_{}sym_{}noise_{}_eq{}_wd{}_t{}_soft{}".format(
        ...
        dual_opt = optax.sgd(learning_rate=args.dual_lr)
        equiv_state = dual_opt.init(equiv)
        ...
        dual_grad = jax.grad(neg_loss, argnums=2)

        @ objax.Jit
        @ objax.Function.with_vars(model.vars()+opt.vars())
        def train_op(x, y, lr, equiv, equiv_state):
            g, v = grad_and_val(x, y, equiv)
            opt(lr=lr, grads=g)
            dual_g = dual_grad(x, y, equiv)
            updates, equiv_state = dual_opt.update(dual_g, equiv_state, equiv)
            equiv = optax.apply_updates(equiv, updates)
            return v, equiv, equiv_state
        ...
        print("First eqcoeff {}".format(
            ",".join([f"{eq:.2f}" for eq in equiv])
        ))
        for epoch in pbar:
            ...
            for x, y in trainloader:
                l, equiv, equiv_state = train_op(
                    jnp.array(x), jnp.array(y), lr, equiv, equiv_state)
