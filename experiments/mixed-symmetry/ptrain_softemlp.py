...
def main(args):
    ...
    for trial in range(args.trials):
        ...
        # Initialize dataset with 1024 examples
        if args.noise_std == 0:
            if data == "inertia":
                dset = ModifiedInertia(
                    3072, noise=args.noise, axis=args.axis, sign=args.sign)
            elif data == "se3":
                dset = SyntheticSE3Dataset(
                    3072, for_mlp=True if args.network.lower() == "mlp" else False,
                    noisy=args.noisy, noise=args.noise, sym=args.sym, sign=args.sign)
        else:
            if data == "inertia":
                dset = RandomlyModifiedInertia(3072, noise_std=args.noise_std)
        if args.sign == 0:
            if data == "inertia":
                ood = ModifiedInertia(
                    1024, noise=args.noise, axis=args.axis, shift=args.ood_shift)
            elif data == "se3":
                ood = SyntheticSE3Dataset(1024,
            ...
            if data == "inertia":
                ood = ModifiedInertia(
                    1024, noise=args.noise, axis=args.axis, sign=-args.sign)
            elif data == "se3":
                ood = SyntheticSE3Dataset(1024,
                                          for_mlp=True if args.network.lower() == "mlp" else False,
                                          noisy=args.noisy, noise=args.noise,
                ...
                ood = None
        if data != "motion":
            split = {'train': -1, 'val': 1024, 'test': 1024}
        ...
        with all_vars.replicate():
            for epoch in pbar:
                ...
                rglr_update_cond = args.auto_equiv and epoch+1 > args.adjust_equiv_at-100 and epoch+1 <= args.adjust_equiv_at+1
                if not args.logoff or rglr_update_cond:
                    rglr1_list, rglr2 = equiv_regularizer(
                        except_bias=args.no_bias_regular)
