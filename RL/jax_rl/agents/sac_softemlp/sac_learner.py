...
from jax_rl.agents.sac_softemlp import actor, critic, temperature
...
def _update_jit(sac: ActorCriticTemp, batch: Batch, discount: float,
    ...
    sac, critic_info = critic.update(
        sac, batch, discount, True, critic_basic_wd, critic_equiv_wd)
    if update_target:
        sac = critic.target_update(sac, tau)
    sac, actor_info = actor.update(sac, batch, actor_basic_wd, actor_equiv_wd)
    sac, alpha_info = temperature.update(
        sac, actor_info['entropy'], target_entropy)
...
class SoftSACLearner(object):
    def __init__(self,
        ...
        if rpp_policy:  # Use RPP-EMLP policy
            if action_space == "discrete":
                actor_def = policies.SoftEMLPSoftmaxPolicy(state_rep, action_rep, symmetry_group, hidden_dims,
                                                           state_transform=state_transform, inv_action_transform=inv_action_transform,
                                                           gnl=gnl)
            else:
                ch = hidden_dims if middle_rep is None else len(
                    hidden_dims)*[middle_rep]
                actor_def = policies.SoftEMLPNormalTanhPolicy(state_rep, action_rep, action_std_rep, symmetry_group, ch,
            ...
            if action_space == "discrete":
                ...
                actor_def = policies.NormalTanhPolicy(
                    hidden_dims, action_dim, small_init=small_init)
        ...
        if rpp_value:
            ch = hidden_dims if middle_rep is None else len(
                hidden_dims)*[middle_rep]
            critic_def = critic_net.SoftEMLPDoubleCritic(state_rep, action_rep, symmetry_group, ch,
    ...
    def set_state(self, state):
        self.sac.set_state(state)

    
    def get_current_state(self):
        return self.sac.get_current_state()
    ...
    def sample_actions(self,
        ...
        rng, actions = policies.sample_actions(self.sac.rng,
                                               ...
                                               self.standardizer(
                                                   observations) if self.standardizer is not None else observations,
                                               temperature)
