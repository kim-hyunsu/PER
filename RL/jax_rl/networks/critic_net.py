...
class DiscreteCritic(nn.Module):
    ...
    def __call__(self, observations: jnp.ndarray,
        ...
        inputs = jnp.concatenate(
            [observations, actions.reshape(observations.shape[0], 1)], -1)
        critic = MLP((*self.hidden_dims, 1))(inputs)
        return jnp.squeeze(critic, -1)
...
class _RPPDoubleCritic(nn.Module):
    ...
    def __call__(self, observations: jnp.ndarray,
        ...
        state, actions = self.state_transform(
            observations), self.action_transform(actions)
...
class _SoftEMLPDoubleCritic(nn.Module):
    critic1: nn.Module
    critic2: nn.Module
    state_transform: callable
    action_transform: callable
