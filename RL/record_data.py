...
def main(_):
    ...
    kwargs['action_space'] = 'continuous'#environment_symmetries[FLAGS.env_name]['action_space']
    algo = kwargs.pop('algo')
    replay_buffer_size = kwargs.pop('replay_buffer_size')
    if algo == 'sac':
        agent = SACLearner(FLAGS.seed,
                           env.observation_space.sample()[np.newaxis],
                           np.asarray(env.action_space.sample())[None], **kwargs)
    else:
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if done:
            replay_buffer.restarts[replay_buffer.insert_index]=1
            observation, done = env.reset(), False
        ...
        if i % FLAGS.eval_interval == 0:
            if (i//FLAGS.eval_interval) % 10 ==0 and i<9e5:
                replay_buffer.save(f"records_{FLAGS.env_name}.csv")
